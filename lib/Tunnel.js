...
module.exports = class Tunnel extends EventEmitter {
  constructor(opts = {}) {
    ...
    this.closed = false;
    if (!this.opts.host) {
      this.opts.host = 'https://tunnel.dev.fusebit.io';
    }
  }

  _getInfo(body) {
    /* eslint-disable camelcase */
    const { id, ip, port, url, cached_url, max_conn_count, isTls, certificate, OverrideTunnelIp } = body;
  ...
  _init(cb) {
    ...
    const params = {
      responseType: 'json',
      headers: {
        authorization: opt.secret || process.env.FUSETUNNEL_SECRET || '',
      },
    };
    ...
    if (opt.subdomain) {
      searchParams.set("customSubdomain", opt.subdomain)
    }

    (function getUrl() {
      axios
        .get(baseUri+"?"+searchParams.toString(), params)
        .then((res) => {
          const body = res.data;
        ...
        .catch((err) => {
          if (err.response.status === 401) {
            console.log('Unauthenticated, set --secret with the password to access this fusetunnel deployment.');
            return;
          }
